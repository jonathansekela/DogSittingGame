...
#region constructors
	def __init__(self, direction, action, screen_dest):
		...
		sf.createAnimationListRight(self.animation_list, self.ANIMATION_STEPS, self.sprite_sheet_right, self.SPRITE_WIDTH, self.SPRITE_HEIGHT, self.BLACK)
	...
	def update_animation(self, screen):
		...
		if current_time - self.last_update >= self.animation_cooldown:
			...
			if self.frame >= len(self.animation_list[self.action]):
				self.frame = 0
		#show frame image
		screen.blit(self.animation_list[self.action][self.frame], self.screen_dest)
#endregion
